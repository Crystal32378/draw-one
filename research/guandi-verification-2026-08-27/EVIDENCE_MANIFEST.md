...
`slip_regions.py` 逐行切分：marker=「第N」獨立行（允許 markdown 裝飾）；marker 後內容歸該籤直至下一 marker；頁首無主文字 excluded；編號衝突或缺 marker → fail closed（該頁對該籤不提供 evidence）。verify 與 variant analyzer 共用此 boundary。hostile regression：`regression_test.py` 全 PASS。
